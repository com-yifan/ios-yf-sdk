...
- (void)viewDidLoad {
    ...
    self.btnChange.frame = CGRectMake(50, 280, width, 40);
    self.labNotify.frame = CGRectMake(0,320, self.view.frame.size.width, 40);
    self.textV.frame = CGRectMake(0, 380, self.view.frame.size.width, self.view.frame.size.height - CGRectGetMaxY(self.labNotify.frame) - 20);
}
...
- (UIButton *)btnChange {
    if (!_btnChange) {
        ...
        _btnChange.layer.cornerRadius = 5.f;
        _btnChange.layer.masksToBounds = YES;
        _btnChange.layer.borderWidth = 2.f;
        _btnChange.layer.borderColor = UIColor.systemBlueColor.CGColor;
        [_btnChange setTitle:@"切换VC" forState:UIControlStateNormal];
        [_btnChange setTitleColor:[UIColor systemBlueColor] forState:UIControlStateNormal];
        [_btnChange addTarget:self action:@selector(changeVC) forControlEvents:UIControlEventTouchUpInside];
